...
// Final resting position of the camera (x, y, z)
#define CAM_FINAL_LOOK_FROM_POS     0, 70, -35
// The position that the camera should always look at (x, y, z)
#define CAM_LOOK_TO_POS             0, 0, -10

// Text placement positions (x, y)
#define TURNS_LEFT_LABEL_TEXT       -60, 55    // Left aligned
#define TURNS_LEFT_LABEL_NUMBER     -15, 55   // Left aligned
#define CURRENT_ROUND_LABEL_TEXT    -60, 2    // Right aligned
#define CURRENT_ROUND_LABEL_NUMBER  -25, 2   // Right aligned

MyWorld::MyWorld() : World(), GameGrid(0), GameGridWidth(6), GameGridHeight(6), GameState(eGameState_Running), 
    CamSwing(true), StartCamSwingPos(30,150,-60), CamSwingTime(0), FinalCamSwingPos(CAM_FINAL_LOOK_FROM_POS)
{
}
...
void MyWorld::Init()
{
    ...
    LookTo = CIwFVec3(CAM_LOOK_TO_POS);
    ...
    label->Init();
    label->setPosition(TURNS_LEFT_LABEL_TEXT, 0);
	label->setColour(220, 60, 180, 255);
	label->setScale(0.9);
    ...
    TurnsLeftLabel->Init();
	TurnsLeftLabel->setColour(255, 255, 255, 255);
	TurnsLeftLabel->setScale(0.9);
    TurnsLeftLabel->setPosition(TURNS_LEFT_LABEL_NUMBER, 0);
    TurnsLeftLabel->setFont(g_pResources->Font);
	TurnsLeftLabel->setAligment(IW_GX_FONT_ALIGN_RIGHT, IW_GX_FONT_ALIGN_TOP);
    AddActor(TurnsLeftLabel);

    // Create current round label text
    label = new Label();
    label->Init();
    label->setPosition(CURRENT_ROUND_LABEL_TEXT, 0);
    label->setColour(220, 60, 180, 255);
	label->setScale(1.2);
    label->setFont(g_pResources->Font);
	label->setText("ROUND:");
    ...
    RoundLabel->Init();
	RoundLabel->setScale(1.2);
	RoundLabel->setColour(255, 255, 255, 255);
    RoundLabel->setPosition(CURRENT_ROUND_LABEL_NUMBER, 0);
	...
	GameOverLabel->setColour(220, 60, 180, 255);
    GameOverLabel->setAligment(IW_GX_FONT_ALIGN_CENTRE, IW_GX_FONT_ALIGN_BOTTOM);
